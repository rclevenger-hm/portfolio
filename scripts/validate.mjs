...
for(const t of ["Principal Site Reliability Engineer","Oracle","Happy Money","NextVR","Local Corporation","Sports Gamecast","OCI Ephemeral Resource Janitor","Kafka on OCI","case-studies/oci-ephemeral-resource-janitor","Public portfolio boundary"]) if(!llms.includes(t)){console.error("llms.txt missing",t);bad=true;}
...
if(/ghp_[A-Za-z0-9]+|github_pat_[A-Za-z0-9_]+/.test(worker+admin+html+llms+caseStudy)){console.error("possible GitHub token embedded in source");bad=true;}
