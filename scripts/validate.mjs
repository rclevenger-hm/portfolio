...
const required=["index.html","styles.css","experience.css","resume/index.html","resume/resume.css","resume/evidence.html","resume/evidence.css","data/profile.json","llms.txt","sitemap.xml","case-studies/case-study.css","case-studies/oci-ephemeral-resource-janitor/index.html","admin/github/index.html","admin/github/app.js","admin/github/analytics.css","analytics-worker/index.js","analytics-worker/schema.sql","analytics-worker/wrangler.toml"];
...
const evidence=readFileSync(resolve(root,"resume/evidence.html"),"utf8");
const evidenceCss=readFileSync(resolve(root,"resume/evidence.css"),"utf8");
...
for(const t of ["Roger Clevenger","Principal Site Reliability Engineer","Operating profile","Selected experience","Inspectable engineering evidence","OCI Ephemeral Resource Janitor","Kafka on OCI","Identity Document Async Pipeline","Reliability evidence map","evidence.html","Public resume boundary","Print / save PDF","../data/profile.json"]) if(!resume.includes(t)){console.error("resume missing",t);bad=true;}
...
for(const t of ["Reliability evidence map","Decision model","Capability → evidence","OCI Ephemeral Resource Janitor","Kafka on OCI","Identity Document Async Pipeline","Sports Gamecast","Failure mode","Verification","Boundary","Public evidence boundary"]) if(!evidence.includes(t)){console.error("evidence map missing",t);bad=true;}
for(const t of ["decision-path","capability-matrix","system-card","prefers-reduced-motion","@media print"]) if(!evidenceCss.includes(t)){console.error("evidence CSS missing",t);bad=true;}
...
if(/ghp_[A-Za-z0-9]+|github_pat_[A-Za-z0-9_]+/.test(worker+admin+html+llms+caseStudy+resume+evidence)){console.error("possible GitHub token embedded in source");bad=true;}
...
if(resumeBytes>38000){console.error("resume budget exceeded",resumeBytes);bad=true}else console.log(`resume assets: ${resumeBytes}/38000 bytes`);
const evidenceBytes=statSync(resolve(root,"resume/evidence.html")).size+statSync(resolve(root,"resume/evidence.css")).size+statSync(resolve(root,"resume/resume.css")).size;
if(evidenceBytes>42000){console.error("evidence-map budget exceeded",evidenceBytes);bad=true}else console.log(`evidence map assets: ${evidenceBytes}/42000 bytes`);
